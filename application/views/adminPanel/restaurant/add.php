...
<div class="row">
  <div class="col-md-12">
    <div class="card">
      ...
      <div class="card-body">
        <?= form_open($url.'/add'.(($this->input->get('day')) ? '?day='.$this->input->get('day') : ''), 'id="validateForm" name="validateForm"') ?>
        <div class="row">
          <div class="col-md-4">
            <h4 class="card-title">
            <?= form_label('Day', 'day', 'class="col-form-label"') ?>
            </h4>
            <select class="selectpicker" data-size="7" data-style="btn btn-primary btn-round" title="Single Day" id="day" name="day" required="true" onchange="window.location = '<?= base_url($url) ?>/add?day=' + this.value" >
              ...
              <option value="<?= $day ?>" <?= (set_value('day') == $day || $this->input->get('day') == $day) ? 'selected' : '' ?>><?= $day ?></option>
              <?php endforeach ?>
            </select>
            <h4 class="card-title">
          ...
          <div class="col-md-4">
            ...
            <select class="selectpicker" data-style="btn btn-info btn-round" multiple title="Choose Items" name="avail_items[]" id="avail_items" required="true">
              ...
              <option value="<?= e_id($item['id']) ?>"
                <?= ((set_value('avail_items') && in_array(e_id($item['id']), set_value('avail_items')))) ? 'selected' : '' ?>>
                <?= $item['title'] ?>
              </option>
              <?php endforeach ?>
            </select>
          ...
          <div class="col-md-4">
            ...
            <div class="card-body ">
              <div class="form-group">
                <input type="text" id="from_time" name="from_time" class="form-control timepicker" value="<?= set_value('from_time') ?>" required="true" />
              </div>
            </div>
            <div class="card-header ">
              <h4 class="card-title">
            ...
            <div class="card-body ">
              <div class="form-group">
                <input type="text" id="to_time" name="to_time" class="form-control timepicker" value="<?= set_value('to_time') ?>" required="true" />
              </div>
            </div>
          </div>
